...
| 메모리 기반 학습 | 학습 표본을 저장해 두었다가 예측할 때 직접 사용 |
| 지역적 근사 | 새 관측값 주변의 일부 학습자료로만 예측 |
| 비모수적 접근 | 특정 확률분포나 함수 형태를 미리 가정하지 않음 |
| 다수결·산술평균 | 분류와 회귀에서 이웃의 답을 결합하는 기본 방식 |
...
- **인스턴스 기반 학습:** 개별 훈련 사례(instance)를 직접 참조해 새 데이터의 답을 정한다.
- **게으른 학습:** 즉시학습(eager learning)처럼 미리 모형을 만들지 않고, 예측 요청이 들어왔을 때 이웃을 탐색한다.

> **암기:** 학습 단계에서는 저장, 예측 단계에서는 거리 계산.
...
교재의 목적과 특징을 시험형 문장으로 정리하면 다음과 같다.

- 수치형 종속변수면 **회귀모델**, 범주형 종속변수면 **분류모델**로 사용한다.
- $k$는 1 이상의 자연수다.
- $k$가 작을수록 과적합, 클수록 과소적합 경향이 있다.
- 모델 성능은 $k$값에 민감하므로 검증 데이터나 교차검증으로 선택한다.
- 가까운 이웃을 정하려면 거리를 계산하므로 정규화·표준화한 데이터가 권장된다.
...
- **이상치 처리:** $k$개 이웃까지의 거리 분포와 임계값으로 이상치를 판별·처리한다.
- **결측치 처리:** 결측값이 있는 행을 평가 데이터처럼 두고 이웃의 값으로 다중대치할 수 있다.
- **금융:** 유사고객 기반 신용도 평가, 유사거래 기반 사기거래 판별
- **생물정보학:** 유사 유전자 서열을 이용한 신규 유전자 기능 예측

데이터의 국소 구조가 중요하고 변수 수가 너무 많지 않을 때 유용하다. 반대로 특성이 많아 모든 관측값 사이 거리가 비슷해지면 이웃의 의미가 약해진다.

결측치 대치에 k-NN을 사용할 때는 목표변수 정보가 전처리에 새어 들어가지 않도록 학습 데이터 안에서 이웃과 대치 규칙을 정한다.
...
인스턴스(instance)는 구체적 사례, 데이터 포인트(data point), 학습 표본(training sample)을 가리키는 말이다. 인스턴스 기반 학습의 세 특징은 다음과 같다.

1. **메모리 기반 학습:** 모든 학습 데이터 포인트를 메모리에 저장한다.
2. **지역적 근사:** 평가 데이터 주변의 $k$개 학습 데이터에 의존한다.
3. **비모수적 접근:** 특정 확률분포나 함수 형태를 가정하지 않는다.
...
교재 표기대로 두 점 $p=(p_1,\ldots,p_n)$와 $q=(q_1,\ldots,q_n)$의 거리는 다음과 같다.
...
예를 들어 점 $A=(1,2)$와 $B=(4,6)$이면 맨해튼 거리는 $3+4=7$, 유클리드 거리는 $\sqrt{3^2+4^2}=5$다. 같은 자료라도 거리 정의에 따라 이웃 순서가 달라질 수 있다.
...
교재의 분류 예측 절차는 다음과 같다.

1. 학습 데이터를 준비한다.
2. 평가 데이터와 독립변수 기준으로 가장 가까운 $k$개의 학습 이웃을 찾는다.
3. 이웃의 종속변수를 모아 최빈 범주를 반환한다.

![3-NN 분류에서 가까운 세 이웃의 다수결](images/ch17_knn_classification.png)

*그림: 별표 관측값의 가까운 세 이웃 중 원이 두 개이므로 원 범주로 분류 — 원문 도식만 잘라 수록.*

이진분류에서 짝수 $k$는 동점이 생길 수 있어 홀수 $k$를 자주 사용한다. 다중분류나 거리 가중 투표에서는 홀수라도 동점이 가능하므로 최인접 클래스, 사전 비율 등 동점 규칙을 미리 정한다.

### 회귀모델

교재의 회귀 예측 절차도 이웃 탐색까지는 같고, 마지막에 이웃의 종속변수를 산술평균한다.

$$
\hat y=\frac1k\sum_{i\in N_k(x)}y_i
$$

![3-NN 회귀에서 이웃 목표값 50·55·51의 평균](images/ch17_knn_regression.png)

*그림: $(50+55+51)/3=52$이므로 새 관측값의 예측값은 52 — 원문 도식만 잘라 수록.*

교재는 산술평균 외에 가중평균과 중앙값도 사용할 수 있다고 설명한다. 중앙값은 극단적인 이웃 목표값의 영향을 줄이고, 거리 가중평균은 더 가까운 이웃에 큰 영향을 준다.
...
교재가 제시한 지표를 공식까지 한 번에 정리하면 다음과 같다.

| 목적 | 지표 | 공식·해석 |
|---|---|---|
| 분류 | Accuracy | $(TP+TN)/(TP+TN+FP+FN)$ |
| 분류 | Precision | $TP/(TP+FP)$ |
| 분류 | Recall | $TP/(TP+FN)$ |
| 분류 | F1 | $2PR/(P+R)$ |
| 회귀 | MSE | $\frac1n\sum(y_i-\hat y_i)^2$ |
| 회귀 | RMSE | $\sqrt{MSE}$ |
| 회귀 | MAE | $\frac1n\sum|y_i-\hat y_i|$ |
| 회귀 | MAPE | $\frac{100}{n}\sum\left|\frac{y_i-\hat y_i}{y_i}\right|$ |

MAPE는 실제값 $y_i=0$에서 정의되지 않고, 0에 가까우면 값이 과도하게 커지는 한계가 있다.
...
교재가 특히 강조한 두 항목은 다음과 같다.

- **$k$값 선택:** 과적합이 우려되면 $k$를 늘리고, 과소적합이 우려되면 $k$를 줄인다. 최종 선택은 검증 성능으로 한다.
- **거리용 데이터:** 독립변수의 단위·스케일 영향을 줄이기 위해 정규화된 데이터를 사용한다. 범주형 변수를 단순 Label encoding하면 숫자 간 가짜 거리와 순서가 생길 수 있으므로 원-핫 인코딩이 권장된다.

스케일러와 인코더는 전체 데이터가 아니라 학습 데이터에만 적합하고 검증·시험 데이터에는 변환만 적용해야 데이터 누출을 막을 수 있다.
