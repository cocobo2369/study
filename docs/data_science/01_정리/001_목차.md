...
# 모집단과 표본
- 모집단 관계를 가정 → 표본으로 추정 → 검증 → 가정한 모집단 모형을 수정
  - "모집단 관계 가정"은 마치 원피스 처럼 찾아가는 과정인 것이다.
  1. 대상을 분석 하고싶음 - 이 때 모집단의 "개념"이 형성됨
    - 모집단의 모든 데이터를 가질 수도 없으므로 개념적으로 경계를 갖는 것.
  2.  표본으로 모델링
  3.  모집단 관계 가정을 수정

## 잔차(Residual)
- 표본의 종속변수 vs 표본의 모델링 값 차이
- 표본의 실제 관측값 $Y_i$와 표본회귀식의 적합값 $\hat Y_i$의 차이
- 표본 데이터로 계산할 수 있음
$$
e_i=Y_i-\hat Y_i
$$
...
## 오차항(Error)
- 모집단의 종속변수 vs 모집단 모델링 값 차이
- 개별 관측값 $Y_i$와 모집단 회귀식이 나타내는 조건부 평균 $E(Y_i\mid X_i)$의 차이
- 모집단의 실제 회귀관계를 모르므로 직접 관측할 수 없음
$$
\epsilon_i=Y_i-E(Y_i\mid X_i)
$$
...
| 구분 | 평가 지표 |
|---|---|
| 회귀 모델 평가 | $MSE$, $RMSE$, $MAE$, $MAPE$, $R^2$ |
| 분류 모델 평가 (혼동행렬) | Accuracy, Precision, Recall, F1-score |
| 트리 모델 평가 | Tree: Gini, Entropy<br>Random Forest: Voting, Ensemble |
| 군집 모델 평가 | Silhouette, Inertia(응집도) |
